...
suffix = f'input_size{input_size}-hidden_size{hidden_size}-num_layers{num_layers}-dropout{dropout}-look_back_window{look_back_window}-lr{lr}-step_size{step_size}-gamma{gamma}'
writer = SummaryWriter(f'runs/LSTM/{time}_{suffix}')
...
for epoch in range(1, num_epochs + 1):
    ...
    if (len(ret_deque) >= patience and max(ret_deque) == ret_deque[0]) or epoch == num_epochs:
        ...
        torch.save(para_deque[model_index],
                   f'./checkpoints/LSTM/{time}_{suffix}')
        del para_deque
        break
