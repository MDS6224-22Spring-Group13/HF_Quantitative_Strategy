...
X_train = np.load("../15/feature_train_standard.npy")
X_val = np.load("../15/feature_val_standard.npy")
label = np.load("../15/label_15.npy")
...
y_train = label[:train_size, 0] * 100
y_val = label[train_size:train_size+val_size, 0] * 100
upper = np.percentile(y_train, 100 - per)
lower = np.percentile(y_train, per)
y_train[y_train > upper] = upper
y_train[y_train < lower] = lower

trade_table = np.load("../15/trade_table_15.npy")
trade_table_train = trade_table[:train_size, :]
trade_table_val = trade_table[train_size:train_size+val_size, :]

threshold = 1
spread_open = trade_table_train[:, 1] - trade_table_train[:, 0]
spread_close = trade_table_train[:, 3] - trade_table_train[:, 2]
spread_open_val = trade_table_val[:, 1] - trade_table_val[:, 0]
spread_close_val = trade_table_val[:, 3] - trade_table_val[:, 2]
spread_open[spread_open > threshold] = threshold
spread_close[spread_close > threshold] = threshold
spread_open_val[spread_open_val > threshold] = threshold
spread_open_val[spread_open_val > threshold] = threshold

X_train = np.concatenate(
    [X_train, spread_open.reshape(-1, 1), spread_close.reshape(-1, 1), ], axis=1)
X_val = np.concatenate(
    [X_val, spread_open_val.reshape(-1, 1), spread_close_val.reshape(-1, 1)], axis=1)

with open('temp.npz', 'wb') as f:
    np.savez(f, X_train=X_train, X_val=X_val, y_train=y_train, y_val=y_val)
