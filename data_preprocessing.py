...
y = np.load('./dataset/15/label_15.npy')
...
def process_data(X, y, TT, val_size, test_size, threshold=1, random_state=42):
    ...
    X_test = (X_test - np.concatenate([X_train, X_val], axis=0).mean(
    ...
    y_test = (y_test - np.concatenate([y_train, y_val], axis=0).mean(
        axis=0)) / np.concatenate([y_train, y_val], axis=0).std(axis=0)
